...
fitness_history = []
generations_of_crossover = []
crossover_list = []
generations_of_crossover_average = []

iterations = 10
step_size = 0.01

unconverged_list = []
crossover_list_unconverged = []
...
for x in range (600):
    ...
    for i in range (iterations):    
        ...
        while counter <= 2000:
            p = evolve(p, target, random_select= crossover_count)
            current_fitness = grade(p, target)
            fitness_history.append(current_fitness)
            counter +=1
            ...
            if current_fitness <= 0:
                break
        if counter < 2000:
            generations_of_crossover.append(generation_counter)
        ...
        if i == iterations -1:
            ...
            print(generations_of_crossover)
            print(unconverged_counter)
            if len(generations_of_crossover) != 0:
                print("mean: ", mean(generations_of_crossover))
                generations_of_crossover_average.append(mean(generations_of_crossover))
                crossover_list.append(crossover_count)
                generations_of_crossover = []
            if unconverged_counter != 0:
                crossover_list_unconverged.append(crossover_count)
                unconverged_list.append(unconverged_counter)

    crossover_count = crossover_count - step_size
    if crossover_count <=0:
        break
print(crossover_list)
plt.title("Crossover Percentage vs Average Generations to Fitness of 0")
plt.xlabel("Crossover Percentage")
plt.ylabel("Average Generations to Fitness of 0")
plt.plot(crossover_list, generations_of_crossover_average)
plt.show()

plt.title("Crossover Percentage vs Unconverged Runs")
plt.xlabel("Crossover Percentage")
plt.ylabel("Unconverged Runs Within Threshold")
print(unconverged_list)
plt.plot(crossover_list_unconverged, unconverged_list )
plt.show()
